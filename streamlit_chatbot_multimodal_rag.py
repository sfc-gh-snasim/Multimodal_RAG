...
def config_options():
    ...
    st.sidebar.selectbox(
        "Select your model:", ("claude-4-sonnet", "pixtral-large"), key="model_name"
    )

    st.sidebar.selectbox(
        "Select number of docs to retrieve:",
        CHUNK_OPTIONS,
        key="num_chunks",
...
def create_prompt_multimodal(myquestion):
    ...
    if NUM_CHUNKS == 1:
        ...
        image_placeholders_str = (
            ", ".join(image_placeholders[:-1]) + f", and {image_placeholders[-1]}"
        )
...
def main():
    ...
    try:
        docs_available = session.sql(
            f"ls {SOURCE_DOCS_STAGE}/{SOURCE_DOCS_PATH}"
        ).collect()
    except SnowparkSQLException as e:
        st.markdown(
            f"Error listing documents, ensure you have run the notebooks first to create the stage\n{e}"
        )
        return
